...
parent_doi  := dhgzh-arp63
rdm_api_url := https://data.caltech.edu/api/records
this_doi    := $(shell curl -s $(rdm_api_url)/$(parent_doi) | jq -r .pids.doi.identifier)
...
update-doi: vars
	sed -i .bak -e '/doi:/ s|doi: .*|doi: $(this_doi)|' CITATION.cff
